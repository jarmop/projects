...
for (let i = 0; i < defaultSociety.lifeExpectancy; i++) {
  defaultSociety.population.push(10);
}
defaultSociety.birthRate = sum(defaultSociety.population) * portionOfWomen *
...
export function Overview() {
  ...
  const totalPopulation = sum(society.population);
  ...
  function increaseYear() {
    ...
    let deaths = society.population[ageOfNaturalDeath - 1];
    let sumOfAgesOfDying = deaths * ageOfNaturalDeath;

    const newPopulation = [births, ...society.population].slice(0, -1);
    ...
    for (
      ...
      if (peopleToStarve >= newPopulation[age]) {
        peopleToStarve -= newPopulation[age];
        deaths += newPopulation[age];
        sumOfAgesOfDying += newPopulation[age] * age;
        newPopulation[age] = 0;
      } else {
        newPopulation[age] -= peopleToStarve;
